document.addEventListener("DOMContentLoaded", function () {
    ...
    paragraphs.forEach((p) => {
        ...
        const randomRotation = -1 + Math.random() * 2; // Rotate between -1deg and 1deg
        const randomFontSize = 14 + Math.random() * 4; // Font size between 14px and 18px
        const randomWidth = Math.min(100, 60 + Math.random() * 40) + "%"; // Width between 60% and 100%, with a max limit
